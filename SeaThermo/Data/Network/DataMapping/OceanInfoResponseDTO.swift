//
//  OceanInfoResponseDTO.swift
//  SeaThermo
//

import Foundation

struct OceanInfoResponseDTO: Decodable {
    let retList: [OceanInfoItemDTO]
    
    func toDomain() -> OceanResponse {
        return OceanResponse(list: retList.map { $0.toDomain() })
    }
}

extension OceanInfoResponseDTO {
    struct OceanInfoItemDTO: Decodable {
